...
import AsbestosCigaretteAtomPluginView from './fronkensteen-atom-repl-view';
...
export default {
  ...
  asbestosCigaretteAtomPluginView: null,
  modalPanel: null,
  subscriptions: null,
  passPhrase: null,
	activeExpression:null,
  activate(state) {
    this.asbestosCigaretteAtomPluginView = new AsbestosCigaretteAtomPluginView(state.asbestosCigaretteAtomPluginViewState);
    this.modalPanel = atom.workspace.addModalPanel({
      item: this.asbestosCigaretteAtomPluginView.getElement(),
      visible: true
    });
    this.asbestosCigaretteAtomPluginView.setModalPanel(this.modalPanel);
    this.asbestosCigaretteAtomPluginView.setParent(this);
  ...
  deactivate() {
    ...
    this.asbestosCigaretteAtomPluginView.destroy();
  },

  serialize() {
    return {
      asbestosCigaretteAtomPluginViewState: this.asbestosCigaretteAtomPluginView.serialize()
    };
  },
	...
	finddoc(){
		let editor
		let offset
		let passPhrase = this.asbestosCigaretteAtomPluginView.getPassPhrase();
		if(passPhrase === ""){
			this.modalPanel.show();
			return;
		}
		let server = this.asbestosCigaretteAtomPluginView.getHost();
		if(server === ""){
			this.modalPanel.show();
			return;
		}
